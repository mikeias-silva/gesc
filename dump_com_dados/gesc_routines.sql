...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `dadoscrianca` AS select `pessoa`.`nomepessoa` AS `nomepessoa`,`pessoa`.`datanascimento` AS `datanascimento`,`pessoa`.`logradouro` AS `logradouro`,`pessoa`.`bairro` AS `bairro`,`pessoa`.`ncasa` AS `ncasa`,`pessoa`.`complementoendereco` AS `complementoendereco`,`pessoa`.`cpf` AS `cpf`,`pessoa`.`rg` AS `rg`,`pessoa`.`sexo` AS `sexo`,`pessoa`.`emissorrg` AS `emissorrg`,`matriculas`.`idmatricula` AS `idmatricula` from ((`matriculas` join `crianca`) join `pessoa`) where ((`crianca`.`idcrianca` = `matriculas`.`idcrianca`) and (`crianca`.`idpessoa` = `pessoa`.`idpessoa`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
